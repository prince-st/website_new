import { BarChart3, Layers, MessageSquare, Clock, Handshake, Globe2, Sparkles, LucideIcon } from "lucide-react";
...
export function WhyChooseSection() {
  ...
  // badge_top, heading_main, text_highlight
  // cards[].icon (WP image), cards[].title, cards[].desc
  ...
  const rawCards = Array.isArray(data?.cards) && data.cards.some((c: any) => c.title)
    ? data.cards.filter((c: any) => c.title)
    : null;
                ...
                <div key={stat.label} className="group relative flex-1">
                  <div className="relative p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-card border-2 border-border hover:border-primary/30 transition-all duration-500 hover:-translate-y-2 hover:shadow-xl overflow-hidden">
                    ...
                    <div className="relative z-10 text-center">
                      <p className={`font-heading text-2xl sm:text-3xl lg:text-4xl font-bold bg-gradient-to-br ${stat.color} bg-clip-text text-transparent mb-1`}>{stat.value}</p>
