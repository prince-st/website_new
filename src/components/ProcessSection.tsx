...
export function ProcessSection() {
  ...
  // top_badge1, main_heading1, highlight_text1
  // steps[].number, steps[].title, steps[].description

  const badge: string = data?.top_badge1 || "MY PROCESS";
  const headingMain: string = data?.main_heading1 || "How I Turn Ideas into";
  const headingHighlight: string = data?.highlight_text1 || "Reality";

  const rawSteps = Array.isArray(data?.steps) && data.steps.length > 0 ? data.steps : null;
  const steps = rawSteps
    ...
    : [
        ...
        { icon: TestTube, number: "05", title: "Design, Development", description: "Clean, performant, scalable builds — whether it's a Shopify store, AI workflow, or full-stack platform — crafted with precision and attention to detail." },
        { icon: Rocket, number: "06", title: "AI Integration & Automation", description: "Intelligent systems are embedded into your operations — from Agentic RAG pipelines to n8n workflows — turning manual processes into automated growth engines." },
      ];
    ...
    <section id="process" className="py-10 sm:py-12 lg:py-16 bg-slate-100 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12 sm:mb-16 lg:mb-24 animate-fade-in-up">
          <div className="group relative inline-flex items-center gap-2 sm:gap-3 px-4 sm:px-6 lg:px-8 py-3 sm:py-4 rounded-full bg-primary/10 border-2 border-primary/30 backdrop-blur-xl shadow-2xl hover:shadow-primary/40 transition-all duration-500 hover:scale-105 hover:-translate-y-1 overflow-hidden mb-4 sm:mb-6">
            ...
            <div className="relative flex items-center gap-3">
              <Workflow className="w-4 sm:w-5 h-4 sm:h-5 text-primary group-hover:rotate-12 transition-transform" />
          ...
          <p className="text-base sm:text-lg text-muted-foreground">A proven, streamlined process that ensures quality delivery and complete transparency.</p>
        </div>

        {/* Desktop Wave */}
        <div className="hidden lg:block relative max-w-[1400px] mx-auto h-[240px] lg:h-[260px] xl:h-[280px] mt-20 mb-32 xl:mb-20">
          <svg className="absolute inset-0 w-full h-full overflow-visible" viewBox="0 0 1400 300" preserveAspectRatio="none">
            <defs>
            ...
            <path d="M 100 50 C 200 50, 200 250, 300 250 S 400 50, 500 50 S 600 250, 700 250 S 800 50, 900 50 S 1000 250, 1100 250 S 1200 50, 1300 50" fill="none" stroke="hsl(var(--border))" strokeWidth="3" strokeDasharray="10 10" className="opacity-30" />
            <path d="M 100 50 C 200 50, 200 250, 300 250 S 400 50, 500 50 S 600 250, 700 250 S 800 50, 900 50 S 1000 250, 1100 250 S 1200 50, 1300 50" fill="none" stroke="url(#gradient-line)" strokeWidth="3" filter="url(#glow)" />
          </svg>
            ...
            {steps.map((step, index) => {
              ...
              return (
                <div key={index} onMouseEnter={() => setHoveredIndex(index)} onMouseLeave={() => setHoveredIndex(null)}>
                  <div
                    className={cn(
                      "absolute -translate-x-1/2 -translate-y-1/2 rounded-full flex items-center justify-center z-20 transition-all duration-500 bg-card transform-gpu",
                      hoveredIndex === index
                        ? "w-20 h-20 border-[3px] border-primary shadow-[0_0_50px_hsl(var(--primary)/0.8)]"
                        : "w-16 h-16 border-[3px] border-border hover:border-primary/50 shadow-xl"
                    )}
                  ...
                  >
                    ...
                    <span className={cn("relative font-heading font-bold text-base sm:text-lg lg:text-xl transition-all duration-300", hoveredIndex === index ? "text-primary" : "text-muted-foreground")}>
                      {step.number}
                    </span>
                  </div>

                  <div
                    className={cn("absolute -translate-x-1/2 -translate-y-1/2 rounded-full border-2 transition-all duration-300 z-10", hoveredIndex === index ? "w-4 h-4 bg-primary border-primary scale-150 shadow-[0_0_20px_hsl(var(--primary))]" : "w-3 h-3 bg-card border-primary/50 shadow-lg")}
                    style={{ left: dotLeft, top: dotTop }}
                  >
                    {hoveredIndex === index && <span className="absolute inset-0 rounded-full bg-primary animate-ping opacity-75" />}
                  </div>

                  <div
                    className={cn("absolute left-0 flex flex-col items-center justify-center text-center transition-all duration-500 w-40 sm:w-44 lg:w-48", hoveredIndex === index ? "scale-110 z-30" : "z-20")}
                    style={{ left: textLeft, top: dotTop, transform: isEven ? "translate(-50%, -100%) translateY(-24px)" : "translate(-50%, 24px)" }}
                  >
                    <div className={cn("absolute inset-0 -m-3 rounded-2xl backdrop-blur-xl border-2 transition-all duration-500", hoveredIndex === index ? "bg-card/95 border-primary/50 shadow-[0_0_30px_hsl(var(--primary)/0.4)]" : "bg-card/80 border-border/30 shadow-lg opacity-0 group-hover:opacity-100")} />
                    <h3 className={cn("relative font-bold mb-1 text-sm sm:text-base transition-all duration-300", hoveredIndex === index ? "text-primary" : "text-foreground")}>{step.title}</h3>
        ...
        <div className="lg:hidden grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5 md:gap-6">
          {steps.map((step, index) => (
            <div key={index} className="group relative p-5 sm:p-6 rounded-xl sm:rounded-2xl bg-card border-2 border-border transition-all duration-500 hover:border-primary/50 hover:shadow-[0_0_50px_-5px_hsl(var(--primary)/0.6)] hover:-translate-y-3 hover:scale-[1.03] transform-gpu overflow-hidden">
              ...
              <div className="absolute -top-3 sm:-top-4 -left-3 sm:-left-4 w-10 sm:w-12 h-10 sm:h-12 rounded-full bg-gradient-to-br from-primary to-cyan-500 flex items-center justify-center shadow-2xl shadow-primary/40 group-hover:scale-125 transition-all duration-500 border-2 border-primary/30">
                <div className="absolute inset-0 rounded-full bg-gradient-to-br from-white/20 to-transparent" />
                <span className="relative font-heading font-bold text-primary-foreground drop-shadow-lg text-sm sm:text-base">{step.number}</span>
              </div>
              ...
              <h3 className="relative font-heading text-lg sm:text-xl font-semibold mb-1.5 sm:mb-2 group-hover:text-primary transition-colors duration-300">{step.title}</h3>
              <p className="relative text-sm sm:text-base text-muted-foreground leading-relaxed">{step.description}</p>
